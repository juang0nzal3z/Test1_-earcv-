...
##append slices to make each slice wider
	...
	a = ranges(wid3.shape[1], 4)
	a = [int(a[i].split(",")[0]) for i in range(len(a))]
	# b = b[1::4]
	b = a

#~~~~~~~~~~~~~~~~~~~~~~~~Plot
	wid3 = cv2.cvtColor(wid3, cv2.COLOR_BGR2RGB)
	fig = go.Figure()
	fig = make_subplots(rows=1, cols=2)
	widx = wid3.copy()
	widx = cv2.rotate(widx, cv2.cv2.ROTATE_90_COUNTERCLOCKWISE)
	###fig.add_trace(go.Image(z=wid3), row=1, col=1)
	###fig.add_trace(go.Scatter(y=cols,mode='lines',marker=dict(color='LightSkyBlue', size = 120)), row=1, col=1)
...
# add lines using absolute references
	'''for k in range(len(lines)):
		...
		x1=lines[k], y1=wid3.shape[0], line=dict(color='black', width=3), row=1, col=1)'''
	#fig.show()
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
		...
		wid5 = cv2.cvtColor(wid5, cv2.COLOR_BGR2RGB)
		###fig.add_trace(go.Image(z=wid5), row = j ,col = 1)
		###fig.add_trace(go.Scatter(y=cols,mode='lines',marker=dict(color='LightSkyBlue', size = 120)), row = j ,col = 1)
		#fig.add_trace(go.Scatter(y=cols2,mode='lines',marker=dict(color='Blue', size = 120)), row = j ,col = 1)
		#add where the peaks are being called
		bal = cols[c]
		#bal2 = cols2[d]
		###fig.add_trace(go.Scatter(x=c,y=bal, mode='markers', marker=dict(size=8,color='salmon',symbol='cross'),name='Detected Peaks'), row = j ,col = 1)
		#fig.add_trace(go.Scatter(x=d,y=bal2, mode='markers', marker=dict(size=8,color='Blue',symbol='cross'),name='Detected Peaks'), row = j ,col = 1)
	

	# Start --- Implement clustering algo on all peaks
	...
	''' ------------------------------------------------------------------
		Further process the peaks to remove some of the erroneous peak
	...
	all_peaks_final_diff_median = np.median(all_peaks_final_diff)
	prev_ele = all_peaks_final[0]
	threshold = 0.7
	if (all_peaks_final_diff_max-all_peaks_final_diff_median) <= 15:
		threshold = 0.8
	i = 1
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
############################## Best way to consolidate each slice? ########################
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
		

	flat_list = [item for sublist in pks for item in sublist]
	flat_list = np.asarray(flat_list)
	flat_list = np.sort(flat_list)   ##<------ THE ENTIRE SIGNAL BUT FLATTENED ...THERE MAY BE A BETTER WAY
